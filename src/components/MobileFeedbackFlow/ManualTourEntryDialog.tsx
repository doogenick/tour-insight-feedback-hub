...
const ManualTourEntryDialog: React.FC<ManualTourEntryDialogProps> = ({ onCreate }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      let errorMessage = "Failed to save tour to database. Please try again.";
      
      if (error instanceof Error) {
        if (error.message.includes('tour code already exists')) {
          errorMessage = "A tour with this code already exists. Please use a different tour code.";
        } else if (error.message.includes('feedback_gathering_status')) {
