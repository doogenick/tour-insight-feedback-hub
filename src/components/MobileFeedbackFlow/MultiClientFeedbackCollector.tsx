...
const MultiClientFeedbackCollector: React.FC<MultiClientFeedbackCollectorProps> = ({
  ...
  const submitAllFeedback = async () => {
    ...
    try {
      ...
      // Also create local backup as fallback
      ...
      if (successCount > 0) {
        toast({
          title: "Success!",
          description: `${successCount} feedback submissions saved to backend successfully. ${failedCount > 0 ? `${failedCount} saved locally only.` : ''}`,
          duration: 5000
        });
      } else {
        toast({
          title: "Saved Locally",
          description: `All ${completedFeedbacks.length} feedback submissions saved locally and will sync when online.`,
          duration: 5000
        });
      }
