...
app.get('/api/analytics/feedback', (req, res) => {
    ...
    if (tourId)
        filteredFeedback = filteredFeedback.filter(f => f.tour_id === tourId);
    if (startDate) {
...
app.get('/api/analytics/summary', (req, res) => {
    ...
    const ratingCategories = ['overall', 'guide', 'driver', 'food', 'equipment'];
    ...
    res.json({
        ...
        commonThemes,
        timePeriod: {
            start: filteredFeedback.reduce((min, f) => f.submitted_at && new Date(f.submitted_at) < new Date(min) ? f.submitted_at : min, ((_a = filteredFeedback[0]) === null || _a === void 0 ? void 0 : _a.submitted_at) || new Date().toISOString()),
            end: filteredFeedback.reduce((max, f) => f.submitted_at && new Date(f.submitted_at) > new Date(max) ? f.submitted_at : max, ((_b = filteredFeedback[0]) === null || _b === void 0 ? void 0 : _b.submitted_at) || new Date().toISOString())
        }
    });
});
